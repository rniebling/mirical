...
import de.nieblings.webapp.spring.ApplicationSessionListener;
...
public class SessionListener implements HttpSessionListener, ServletContextListener {
	...
	public void contextInitialized(final ServletContextEvent event) {
		//ToDo ??? HibernatePersistenceProviderResolver.register();
	}
	...
	public void contextDestroyed(final ServletContextEvent event) {
		final WebApplicationContext context = WebApplicationContextUtils.getWebApplicationContext(event
				.getServletContext());
		//ToDo ???final SchedulerInitializer initializer = context.getBean(SchedulerInitializer.class);
		//ToDo ???initializer.shutdown();
	}
